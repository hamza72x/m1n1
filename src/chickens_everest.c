...
/*
 * FIXME: M4Pro-guess — cloned from init_t6030_everest verbatim.
 * M4 Pro's P-core is almost certainly a new generation (Apple internal codename
 * unknown publicly at time of writing), so every HID register write below is
 * a speculative reuse. Expected failure modes: silent hang very early in boot,
 * or later instability under load. Replace each reg_set/reg_mask once we have
 * either: (a) Apple's real init sequence from Darwin, or (b) empirical sweeps.
 */
...
/* FIXME: M4Pro-guess — M4 Max; currently identical to t6040. */
void init_t6041_everest(int rev)
{
    init_t6040_everest(rev);
}
